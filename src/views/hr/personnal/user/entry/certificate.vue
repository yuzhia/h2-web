<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['hr:certificate:add']">
          新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hr:certificate:edit']">
          修改
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hr:certificate:remove']">
          删除
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="certificateList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="证件编号" align="center" prop="code" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="certificateRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="证件编号" prop="code">
          <el-input v-model="form.code" placeholder="请输入证件编号" />
        </el-form-item>
...
import {
  ...
  addCertificate,
  updateCertificate
} from '@/api/hr/personnal/certificate'
...
const data = reactive({
  ...
  rules: {
    ...
    code: [{ required: true, message: '证件编号不能为空', trigger: 'blur' }]
  }
})
...
function handleUpdate(row) {
  ...
  getCertificate(_id).then((response) => {
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除员工证件编号为"' + _ids + '"的数据项？')
    .then(function () {
      return delCertificate(_ids)
    })
    .then(() => {
      getList()
